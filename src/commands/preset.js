...
const { preset, rollAllDices, } = require('@nihilncunia/diceroll');
  ...
  run: ({ interaction, client, handler, }) => {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFields(...result.map((item) => ({
        name: item.formula,
        value: `- 전체 결과: **[ ${item.diceTotal} ]**\n- 상세 결과: ${item.diceDetails.map((item2) => `${item2.dice} **[ ${item2.total} ]** (${item2.details.join(',')})\n`)}- 보정: [${item.modDetails.join(',')}]`,
      })))
